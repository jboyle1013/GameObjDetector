...
class ObjectDetector:
            ...
            with serial.Serial(port, baud_rate) as ser:
                while self.running:
                    ...
                    if self.send_gyro_data:
                        # Fetch gyro data
                        self.accel_data, self.gyro_datarad, self.gyro_datadeg = self.dc.get_imu_data()
    ...
    def draw_and_print_info(self, className, confidence, robot_Vals, box, color_image):
        ...
        # font_color = classColor
        font = cv2.FONT_HERSHEY_DUPLEX
        font_thickness = 1
        cv2.putText(color_image, f"{className}: {confidence * 100:.0f}%", org, font, font_size, font_color,
                    font_thickness)
        if "Line" in className or "Zone" in className:
            if className == "RedZone":
                cv2.putText(color_image, f"Distance: {depth:.3f} mm.",
                            mid3, font, font_size, font_color, font_thickness)
                cv2.putText(color_image, f"Direction: {abs(horizontal_angle):.2f} deg {direction}.",
                            mid4, font, font_size, font_color, font_thickness)

            elif className == "YellowLine":
                cv2.putText(color_image, f"Distance: {depth:.3f} mm.",
                            xmid, font, font_size, font_color, font_thickness)
                cv2.putText(color_image, f"Direction: {abs(horizontal_angle):.2f} deg {direction}.",
                        xmid2, font, font_size, font_color, font_thickness)

            else:
                cv2.putText(color_image, f"Distance: {depth:.3f} mm.",
                            mid, font, font_size, font_color, font_thickness)
                cv2.putText(color_image, f"Direction: {abs(horizontal_angle):.2f} deg {direction}.",
                            mid2, font, font_size, font_color, font_thickness)

        else:
            cv2.putText(color_image, f"Distance: {depth:.3f} mm.",
                        bottom, font, font_size, font_color, font_thickness)
            cv2.putText(color_image, f"Direction: {abs(horizontal_angle)   :.2f} deg {direction}.",
                        bbottom, font, font_size, font_color, font_thickness)
    ...
    def start_detection(self):
        ...
        try:
            asyncio.ensure_future(self.capture_imu_data())  # Start capturing IMU data
            asyncio.ensure_future(self.capture_frames())  # Start capturing frames
            asyncio.ensure_future(self.process_frames())  # Start processing frames
            loop.run_forever()
    ...
    async def process_frames(self):
        while self.running:
            if not self.frame_queue.empty():
                ...
                print(f"Gyro Data Degrees: {self.gyro_datadeg}")
                # Check for quit key
    ...
    def on_press(self, key):
        try:
            if key == keyboard.Key.enter or key.char in ['c', 'C']:
